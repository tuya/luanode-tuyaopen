...
#if defined(__linux__) || (defined(LWIP_IPV6_MLD) && (LWIP_IPV6_MLD == 1) && defined(LWIP_STACK))
...
{
#if defined(__linux__) || (defined(LWIP_IPV6) &&  defined(LWIP_STACK) && defined(IPV6_UNICAST_HOPS))
  return opt_setint(L, ps, IPPROTO_IPV6, IPV6_UNICAST_HOPS);
#else
    lua_pushnil(L);
    lua_pushstring(L, "setsockopt ip6_unicast_hops not support");
    return 2;
#endif
}
...
{
#if defined(__linux__) || (defined(LWIP_IPV6) &&  defined(LWIP_STACK) && defined(IPV6_MULTICAST_HOPS))
  return opt_setint(L, ps, IPPROTO_IPV6, IPV6_MULTICAST_HOPS);
#else
    lua_pushnil(L);
    lua_pushstring(L, "setsockopt ip6_multicast_hops not support");
    return 2;
#endif
}
...
{
#if defined(__linux__) || (defined(LWIP_STACK) && defined(IP_MULTICAST_LOOP))
    return opt_setboolean(L, ps, IPPROTO_IP, IP_MULTICAST_LOOP);
#else
    lua_pushnil(L);
    lua_pushstring(L, "setsockopt ip_multicast_loop not support");
    return 2;
#endif
}
...
{
#if defined(__linux__) || (defined(LWIP_STACK) && defined(LWIP_IPV6) && defined(IPV6_MULTICAST_LOOP))
    return opt_setboolean(L, ps, IPPROTO_IPV6, IPV6_MULTICAST_LOOP);
#else
    lua_pushnil(L);
    lua_pushstring(L, "setsockopt ip6_multicast_loop not support");
    return 2;
#endif
}
...
int opt_set_linger(lua_State *L, p_socket ps)
{
    ...
    li.l_onoff = (uint16_t) lua_toboolean(L, -1);
    lua_pushstring(L, "timeout");
    lua_gettable(L, 3);
    if (!lua_isnumber(L, -1))
        luaL_argerror(L, 3, "number 'timeout' field expected");
    li.l_linger = (uint16_t) lua_tonumber(L, -1);
...
{
#if defined(__linux__) || defined(LWIP_STACK)
    return opt_setint(L, ps, IPPROTO_IP, IP_MULTICAST_TTL);
#else
    lua_pushnil(L);
    lua_pushstring(L, "setsockopt ip_multicast_ttl not support");
    return 2;
#endif
}
...
#else
    lua_pushnil(L);
    lua_pushstring(L, "setsockopt ip_multicast_if not support");
    return 2;
#endif
}
...
#else
    lua_pushnil(L);
    lua_pushstring(L, "getsockopt ip_multicast_if not support");
    return 2;
#endif
}
...
{
#if defined(__linux__) || (defined(LWIP_IGMP) && defined(LWIP_STACK))
    return opt_setmembership(L, ps, IPPROTO_IP, IP_ADD_MEMBERSHIP);
#else
    lua_pushnil(L);
    lua_pushstring(L, "add membership unsupport");
    return 2;
#endif
}
...
#else
    lua_pushnil(L);
    lua_pushstring(L, "ip6 add membership unsupport");
    return 2;
#endif
}
...
{
#if defined(__linux__) || (defined(LWIP_IPV6_MLD) &&(LWIP_IPV6==1)&& defined(LWIP_STACK))
...
{
#if defined(__linux__) || (defined(LWIP_IPV6) && (LWIP_IPV6 == 1) && defined(LWIP_STACK))
    return opt_getboolean(L, ps, IPPROTO_IPV6, IPV6_V6ONLY);
#else
    lua_pushnil(L);
    lua_pushstring(L, "ip6 only unsupport");
    return 2;
#endif
}
