...
#else
    lua_pushnil(L);
    lua_pushstring(L, "unport getnameinfo implemented");
    return 2;
#endif
}
...
#else
        lua_pushnil(L);
        lua_pushstring(L, "unsupport gethostname implemented");
        return 2;
#endif
}
...
{
#if defined(__linux__) && defined(CONFIG_IPV6)
    int err;
...
#else
    lua_pushnil(L);
    lua_pushstring(L, "unsupport getpeername");
    return 2;
#endif
}
...
#else
    lua_pushnil(L);
    lua_pushstring(L, "unsupport getsockname");
    return 2;
#endif
}
...
#if (defined(__linux__) && defined(CONFIG_IPV6)) || (defined(LWIP_STACK) && defined(LWIP_IPV6) && (LWIP_IPV6==1))
