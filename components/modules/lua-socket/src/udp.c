...
\*-------------------------------------------------------------------------*/
#if defined(LWIP_STACK) && (LWIP_STACK == 1)
static int extract_address_info(const struct sockaddr *addr, socklen_t addr_len,
                            char *addrstr, size_t addrstr_len, char *portstr, size_t portstr_len) {
    if (addr->sa_family == AF_INET) {
        // IPv4
        struct sockaddr_in *addr_in = (struct sockaddr_in *)addr;
        // 将 IPv4 地址转换为字符串
        inet_ntop(AF_INET, &addr_in->sin_addr, addrstr, addrstr_len);
        // 获取端口号并转换为字符串
        snprintf(portstr, portstr_len, "%d", ntohs(addr_in->sin_port));
    }
        ...
        // 将 IPv6 地址转换为字符串
        inet_ntop(AF_INET6, &addr_in6->sin6_addr, addrstr, addrstr_len);
        // 获取端口号并转换为字符串
        snprintf(portstr, portstr_len, "%d", ntohs(addr_in6->sin6_port));
    }
#endif
    else {
        return ERR_VAL;
...
#if defined(CONFIG_IPV6) || (defined(LWIP_IPV6) && (LWIP_IPV6 == 1))
...
#else
     err = extract_address_info((struct sockaddr *)&addr, addr_len, addrstr, sizeof(addrstr), portstr, sizeof(portstr));
#endif
	if (err) {
        lua_pushnil(L);
        lua_pushstring(L, LUA_GAI_STRERROR(err));
