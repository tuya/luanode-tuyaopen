...
int add_bytes(LUA_ZBUFF_T *buff, const char *source, size_t len)
{
...
//获取某点的颜色
uint32_t get_framebuffer_point(LUA_ZBUFF_T *buff,uint32_t point)
{
    switch (buff->bit)
    {
...
static int l_zbuff_create(lua_State *L)
{
    ...
    LUA_ZBUFF_T *buff = (LUA_ZBUFF_T *)lua_newuserdata(L, sizeof(LUA_ZBUFF_T));
    if (buff == NULL)
    {
        return 0;
    }
    buff->addr = (uint8_t *)lua_zbuff_malloc(len);
    if (buff->addr == NULL)
    {
        lua_pushnil(L);
    ...
    luaL_setmetatable(L, LUA_ZBUFF_TYPE);
    return 1;
}
...
static int l_zbuff_read(lua_State *L)
{
    ...
    char *return_str = (char *)lua_zbuff_malloc(read_num);
    if (return_str == NULL)
    {
        return 0;
    }
...
{
    //LUA_ZBUFF_T **pp = luaL_checkudata(L, 1, LUA_ZBUFF_TYPE);
    // int i;

    luaL_getmetatable(L, LUA_ZBUFF_TYPE);
    lua_pushvalue(L, 2);
    lua_rawget(L, -2);
...
{
    LUA_ZBUFF_T *buff = tozbuff(L);
    lua_zbuff_free(buff->addr);
    return 0;
}

int __zbuff_resize(LUA_ZBUFF_T *buff, uint32_t new_size)
{
	void *p = lua_zbuff_malloc(new_size);
	if (p)
	{
...
static int l_zbuff_copy(lua_State *L)
{
    ...
    {
        LUA_ZBUFF_T *copy_buff = ((LUA_ZBUFF_T *)luaL_checkudata(L, 3, LUA_ZBUFF_TYPE));
...
static int l_zbuff_equal(lua_State *L)
{
    ...
    LUA_ZBUFF_T *buff2 = ((LUA_ZBUFF_T *)luaL_checkudata(L, 3, LUA_ZBUFF_TYPE));
...
{
    luaL_newmetatable(L, LUA_ZBUFF_TYPE); /* create metatable for file handles */
