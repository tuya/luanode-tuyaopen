...
/* channel list mutex */
static MUTEX_HANDLE mutex_channel_list = NULL;

/* recycle list mutex */
static MUTEX_HANDLE mutex_recycle_list = NULL;
...
SEM_HANDLE cond_mainls_sendrecv = NULL;

/* main state communication mutex */
static MUTEX_HANDLE mutex_mainls = NULL;
...
static channel *channel_create( const char *cname ) {
  ...
  tal_mutex_lock( mutex_channel_list );
  ...
  list_init( &chan->send );
  list_init( &chan->recv );
  tal_mutex_create_init( &chan->mutex );
  tal_semaphore_create_init( &chan->can_be_used,0,1 );

  /* release exclusive access to channels list */
  tal_mutex_unlock( mutex_channel_list );
...
static channel *channel_locked_get( const char *chname ) {
 ...
 while ((chan = channel_unlocked_get(chname)) != NULL) {
    // 尝试获取互斥量
    if (tal_mutex_lock(chan->mutex) == 0) {  // 成功获取锁
        // 处理完毕后，可以解锁互斥量
        tal_mutex_unlock(chan->mutex);
        break; // 退出循环
    } else {
        // 获取锁失败，释放主互斥量并等待信号量
        tal_mutex_unlock(mutex_channel_list);  // 释放主互斥量
        tal_semaphore_wait_forever(chan->can_be_used); // 等待信号量
        // 重新获取主互斥量
        tal_mutex_lock(mutex_channel_list);
    }
  }
...
void luaproc_unlock_channel( channel *chan ) {
  ...
  tal_semaphore_post( chan->can_be_used );
  /* release exclusive access to channels list */
  tal_mutex_unlock( mutex_channel_list );
...
void luaproc_recycle_insert( luaproc *lp ) {
  ...
  tal_mutex_lock( mutex_recycle_list );
  ...
  tal_mutex_unlock( mutex_recycle_list );
}
...
static int luaproc_send( lua_State *L ) {
  ...
  if ( dstlp != NULL ) { /* found a receiver? */
    ...
    if ( dstlp->lstate == mainlp.lstate ) {
      ...
      tal_mutex_lock( mutex_mainls );
      tal_semaphore_post( cond_mainls_sendrecv );
      tal_mutex_unlock( mutex_mainls );
    } else {
    ...
    if ( L == mainlp.lstate ) {
      ...
      luaproc_unlock_channel( chan );
      tal_mutex_lock( mutex_mainls );
      tal_semaphore_wait_forever( cond_mainls_sendrecv );
      tal_mutex_unlock( mutex_mainls );
...
static int luaproc_destroy_channel( lua_State *L ) {
 ...
 while ((chan = channel_unlocked_get(chname)) != NULL) {
  ...
  tal_mutex_unlock( chan->mutex );
  tal_mutex_release( chan->mutex );
  tal_semaphore_release( chan->can_be_used );
...
#endif
LUAMOD_API int luaopen_tyos (lua_State *L);
...
static int luaproc_loadlib( lua_State *L ) {
  ...
  /* register luaproc functions */
  int ret = 0;
  ret = tal_mutex_create_init(&mutex_channel_list);
    if ( ret) {
        PR_ERR( "mutex_channel_list create failed" );
        goto luaproc_loadlib;
    }
  ret = tal_mutex_create_init(&mutex_recycle_list);
    if ( ret) {
        PR_ERR( "mutex_recycle_list create failed" );
        goto luaproc_loadlib;
    }
  ret = tal_semaphore_create_init(&cond_mainls_sendrecv,0,1);
    if ( ret) {
        PR_ERR( "cond_mainls_sendrecv create failed" );
        goto luaproc_loadlib;
    }
  ret = tal_mutex_create_init(&mutex_mainls);
    if ( ret) {
        PR_ERR( "mutex_mainls create failed" );
        goto luaproc_loadlib;
    }
  ...
  return 1;
luaproc_loadlib:
    if (mutex_channel_list) {
        tal_mutex_release( mutex_channel_list );
    }
    if (mutex_recycle_list) {
        tal_mutex_release( mutex_recycle_list );
    }
    if (cond_mainls_sendrecv) {
        tal_semaphore_release(cond_mainls_sendrecv );
    }
    if (mutex_mainls) {
        tal_mutex_release(mutex_mainls );
    }
    return -1;
}