...
static void *alien_openlib (lua_State *L, const char *libname) {
  void *lib = lua_capi_dlopen();
  if(lib == NULL) lua_pushstring(L, "rtos lua_capi_dlopen return NULL");
  return lib;
}

static void *alien_loadfunc (lua_State *L, void *lib, const char *sym) {
  if(!lib) lib = RTLD_DEFAULT;
  void *f = lua_capi_dlsym(lib, sym);
  printf("f: %p\n", f);
  if (f == NULL) lua_pushstring(L, "rtos lua_capi_dlsym return NULL");
  return f;
}
...
static int alien_function_call(lua_State *L) {
  ...
  void **args = NULL;
  void **arg_pointers = NULL;  // 用于存储参数指针的数组
  alien_Function *af = alien_checkfunction(L, 1);
  ffi_cif *cif = &(af->cif);
  ...
  if(nargs > 0) {
    args = (void **)tal_malloc(sizeof(void*) * nargs);
    arg_pointers = (void **)tal_malloc(sizeof(void*) * nargs);  // 分配存储参数指针的数组
  }
  ...
  for(i = 0; i < nargs; i++) {
    ...
    switch(af->params[i]) {
    case AT_byte:
      arg = tal_malloc(sizeof(char)); *((char*)arg) = (signed char)lua_tointeger(L, j);
      break;
    case AT_char:
      arg = tal_malloc(sizeof(unsigned char)); *((unsigned char*)arg) = (unsigned char)lua_tointeger(L, j);
      break;
    case AT_short:
      arg = tal_malloc(sizeof(short)); *((short*)arg) = (short)lua_tonumber(L, j);
      break;
    case AT_ushort:
      arg = tal_malloc(sizeof(unsigned short)); *((unsigned short*)arg) = (unsigned short)lua_tonumber(L, j);
      break;
    case AT_int:
      arg = tal_malloc(sizeof(int)); *((int*)arg) = (int)lua_tonumber(L, j);
      break;
    case AT_uint:
      arg = tal_malloc(sizeof(unsigned int)); *((unsigned int*)arg) = (unsigned int)lua_tonumber(L, j);
      break;
    case AT_long:
      arg = tal_malloc(sizeof(long)); *((long*)arg) = (long)lua_tonumber(L, j);
      break;
    case AT_ulong:
      arg = tal_malloc(sizeof(unsigned long)); *((unsigned long*)arg) = (unsigned long)lua_tonumber(L, j);
      break;
    case AT_ptrdiff_t:
      arg = tal_malloc(sizeof(ptrdiff_t)); *((ptrdiff_t*)arg) = (ptrdiff_t)lua_tonumber(L, j);
      break;
    case AT_size_t:
      arg = tal_malloc(sizeof(size_t)); *((size_t*)arg) = (size_t)lua_tonumber(L, j);
      break;
    case AT_float:
      arg = tal_malloc(sizeof(float)); *((float*)arg) = (float)lua_tonumber(L, j);
      break;
    case AT_double:
      arg = tal_malloc(sizeof(double)); *((double*)arg) = (double)lua_tonumber(L, j);
      break;
    case AT_string:
      arg = tal_malloc(sizeof(char*));
    ...
    case AT_refchar:
      arg = tal_malloc(sizeof(char *));
      *((char **)arg) = tal_malloc(sizeof(char));
      **((char **)arg) = (char)lua_tonumber(L, j);
      nrefc++;
      break;
    case AT_refint:
      arg = tal_malloc(sizeof(int *));
      *((int **)arg) = tal_malloc(sizeof(int));
      **((int **)arg) = (int)lua_tonumber(L, j);
      nrefi++;
      break;
    case AT_refuint:
      arg = tal_malloc(sizeof(unsigned int *));
      *((unsigned int **)arg) = tal_malloc(sizeof(unsigned int));
      **((unsigned int **)arg) = (unsigned int)lua_tonumber(L, j);
      nrefui++;
      break;
    case AT_refdouble:
      arg = tal_malloc(sizeof(double *));
      *((double **)arg) = tal_malloc(sizeof(double));
      **((double **)arg) = (double)lua_tonumber(L, j);
      nrefd++;
      break;
    case AT_longlong:
      arg = tal_malloc(sizeof(long long)); *((long long*)arg) = (long long)lua_tonumber(L, j);
      break;
    case AT_ulonglong:
      arg = tal_malloc(sizeof(unsigned long long)); *((unsigned long long*)arg) = (unsigned long long)lua_tonumber(L, j);
      break;
    case AT_callback:
      arg = tal_malloc(sizeof(void*)); *((void**)arg) = (alien_Function *)alien_checkfunction(L, j)->fn;
      break;
    default:
      tal_free(args);
      tal_free(arg_pointers);
      return luaL_error(L, "alien: parameter %d is of unknown type (function %s)", j,
                        af->name ? af->name : "anonymous");
    }
    args[i] = arg;
    arg_pointers[i] = arg;  // 将参数指针存储到数组中
  }
  ...
  for(i = 0; i < nargs; i++) {
    ...
    tal_free(arg_pointers[i]);  // 释放每个参数的内存
  }
