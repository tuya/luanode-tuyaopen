...
#define l_remove(a) tal_fs_remove(a)
#define l_rename(a,b) tal_fs_rename(a,b)
...
static int os_clock (lua_State *L) {
  lua_pushnumber(L, ((lua_Number)tal_system_get_millisecond())/(lua_Number)1000);
  return 1;
}
...
struct tm *rtos_localtime(const time_t *timer)
{
...
//     else {
// #if defined(ENABLE_CELLULAR) && (ENABLE_CELLULAR==1)
//         tkl_cellular_comm_ctrl_get_localtime(&result);
// #endif
//     }
    return &result;
}

struct tm *rtos_gmtime(const time_t *timep)
{
    static struct tm result;
    POSIX_TM_S tm;
    if (timep == NULL) {
...
//           INT64_T timestamp = 0;
// #if defined(ENABLE_CELLULAR) && (ENABLE_CELLULAR==1)
//           tkl_cellular_comm_get_epoch_time(&timestamp);
//         #endif
//           tal_time_gmtime_r((const TIME_T*)&timestamp,&tm);
//           result.tm_year = tm.tm_year;
//           result.tm_mon = tm.tm_mon;
//           result.tm_mday = tm.tm_mday;
//           result.tm_wday = tm.tm_wday;
...
//       }
      return &result;
        ...
        tal_time_gmtime_r((const TIME_T*)&timep,&tm);
        result.tm_year = tm.tm_year;
        result.tm_mon = tm.tm_mon;
        result.tm_mday = tm.tm_mday;
        result.tm_wday = tm.tm_wday;
