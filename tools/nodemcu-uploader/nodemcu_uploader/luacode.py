...
DOWNLOAD_FILE = "fd=io.open('{filename}') print(fd:seek('end', 0)) fd:seek('set', {bytes_read}) print(fd:read({chunk_size})) fd:close()"

PRINT_FILE = "fd=io.open('{filename}') print('---{filename}---') print(fd:read('a')) fd:close() print('---')"

INFO_GROUP = "sys.info()"

LIST_FILES = 'for key,value in pairs(io.list("/")) do print(key,value) end'
# NUL = \000, ACK = \006
RECV_LUA = \
r"""
function recv()
...
REMOVE_ALL_FILES = r"""
for key,value in pairs(io.list()) do io.remove(key) end
"""
